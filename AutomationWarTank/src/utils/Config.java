...
public final class Config {
	...
	private String[] planMineProduction = { "-2-", "-2-", "-2-", "-2-", "-0-", "-0-", "-0-", "-3-", "-3-", "-1-" };
	private String[] planArmoryProduction = { "-1-", "-1-", "-1-", "-1-", "-3-", "-0-", "-2-" };
	...
	private void loadingConfiguration(String configFileName) throws ConfigurationException, ParseException {
		GlobalVars.logger.Logging("Current Config path:" + configPath);
		XMLConfiguration configRead = new XMLConfiguration(configFileName);
		String[] tempPlanArmoryProduction = configRead.getStringArray("ShellsProduction");
		String[] tempPlanMineProduction = configRead.getStringArray("MineProduction");
		int tempSleepInterval = configRead.getInt("SleepingInterval", 5);
		int tempRandomInterval = configRead.getInt("RandomInterval", 5);

		int tempLimitUsingSpecialShell = configRead.getInt("LimitUsingSpecialShell", 30);
		int tempEnemyDurabilityLimitUsingSpecialShall = configRead.getInt("EnemyDurabilityLimitUsingSpecialShall", 200);
		String tempBankProduction = configRead.getString("BankProduction", "-1-");
		int tempEnemyDurabilityLimitUsingShortTimeReload = configRead.getInt(
				"EnemyDurabilityLimitUsingShortTimeReload", 200);
		int tempMaxDurability4UsingRepaer = configRead.getInt("MaxDurability4UsingRepaer", 250);
		...
		String tempFighterLogFileName = configRead.getString("FighterLogFileName", "d:\\reset.txt");
		String[] tempAllied = configRead.getStringArray("Allied");
		for (int i = 0; i < tempAllied.length; i++) {
			tempAllied[i] = tempAllied[i].substring(1, tempAllied[i].length() - 1);
		}
		...
		GlobalVars.logger.Logging("planArmoryProduction:" + outArray(planArmoryProduction));
		GlobalVars.logger.Logging("planMineProduction:" + outArray(planMineProduction));
		GlobalVars.logger.Logging("sleepInterval:" + String.valueOf(sleepInterval));
		GlobalVars.logger.Logging("randomInterval:" + String.valueOf(randomInterval));
		GlobalVars.logger.Logging("limitUsingSpecialShell:" + String.valueOf(limitUsingSpecialShell));
		...
		GlobalVars.logger.Logging("maxDurability4UsingRepaer:" + maxDurability4UsingRepaer);
		...
		GlobalVars.logger.Logging("enableBodylogger.Logging:" + enableBodyLogging);
